...
public class LoginController {
    ...
    public ResponseMessage executeOrder(@RequestBody AuthData authData,
                                        HttpServletRequest request) {

        if (authData.getLogin().equals("admin")
                && authData.getPassword().equals("123456")) {
            request.getSession().setAttribute(
                    LoginFilter.LOGIN_ATTR, LocalDateTime.now()
            );
            return
                    ResponseMessage.okMessage(
                            bar.getBarStatus()
                    );
        } else {
            return
                    ResponseMessage.errorMessage("Wrong authorisation!");
        }
